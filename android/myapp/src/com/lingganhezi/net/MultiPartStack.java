...
public class MultiPartStack extends HurlStack {
	...
	static HttpUriRequest createMultiPartRequest(Request<?> request, Map<String, String> additionalHeaders)
		...
		case Method.PATCH: {
			...
			// return patchRequest;
			throw new IllegalArgumentException("not supprot PATCH Method!");
		}
		default:
	...
	private static void setMultiPartBody(HttpEntityEnclosingRequestBase httpRequest, Request<?> request)
		...
		for (Map.Entry<String, File> entry : fileUpload.entrySet()) {
			// TODO 改成 contenttype 根据 entry 每个key获取
			ContentType ct = ((MultiPartRequest) request).getFileContentType();
			File file = entry.getValue();
			if (ct != null) {
				builder.addBinaryBody(entry.getKey(), file, ct, file.getName());
			} else {

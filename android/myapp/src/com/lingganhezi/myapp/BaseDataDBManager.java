 ...
 * 基础数据db
 * 
 * 会自动拷贝 access目录下basedata.db到/data/data/包名/database/
 * 
 * @author chenzipeng
 *
 */
public class BaseDataDBManager extends SQLiteOpenHelper {
	private static final String DBName = "basedata.db";
	private final static int version = 1;

	public BaseDataDBManager(Context context) {
		this(context, DBName, null, version, null);
	}

	private static final String TAG = BaseDataDBManager.class.getSimpleName();
	 ...
	 *
	 * Write-ahead logging cannot be used with read-only databases so the value
	 * of this flag is ignored if the database is opened read-only.
	 *
	 * @param enabled
		...
		try {
			...
			if (db != null) {
				if (writable && db.isReadOnly()) {
					Method m = db.getClass().getDeclaredMethod("reopenReadWrite");
				...
				db = SQLiteDatabase.openDatabase(path, mFactory, SQLiteDatabase.OPEN_READWRITE, mErrorHandler);
			}
	...
	public void copyDatabase() {
		String databasePath = mContext.getApplicationContext().getDatabasePath(DBName).getAbsolutePath();

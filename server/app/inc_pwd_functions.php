 ...
 * @return    string
 */
function newmail($mid, $userid, $mailto, $type)
{
    ...
    if($type == 'INSERT')
    {
        ...
        if($db->ExecuteNoneQuery($sql))
        {
            ...
            $result = new Result(CODE_SUCCESS,'EMAIL修改验证码已经发送到原来的邮箱请查收');
	       	echo json_encode($result);
			exit();
        }
        else
        {
            $result = new Result(CODE_FAILD,'对不起修改失败，请与管理员联系');
	       	echo json_encode($result);
			exit();
        }
    }
...
function member($mail, $userid)
{
    ...
    $row = $db->GetOne($sql);
    if(!is_array($row)){
    	$result = new Result(CODE_FAILD,'找不到这个注册邮箱');
       	echo json_encode($result);
		exit();
	}
    else return $row;
...
function sn($mid,$userid,$mailto)
{
    ...
    {
        //发送新邮件；
        newmail($mid,$userid,$mailto,'INSERT');
    }
    //10分钟后可以再次发送新验证码；
    elseif($dtime - $tptim > $row['mailtime' ])
		{
		newmail($mid,$userid,$mailto,
	'UPDATE');
    }
    //重新发送新的验证码确认邮件；
    else
    {
    	$result = new Result(CODE_FAILD,'对不起，请10分钟后再重新申请');
       	echo json_encode($result);
		exit();
    }
}
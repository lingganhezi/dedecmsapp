...
if($cfg_mb_open=='N')
{
    sendResult(CODE_FAILD,"系统关闭了会员功能，因此你无法访问此页面！");
    exit();
}
...
function CheckRank($rank=0, $money=0, $needinfo=TRUE)
{
    ...
    else
    {
        if($cfg_mb_reginfo == 'Y' && $needinfo)
        {
            ...
            {
                sendResult(CODE_FAILD,"尚未完成详细资料，请完善...");
            }
        }
        ...
        if($cfg_ml->M_Rank < $rank)
        {
            ...
            sendResult(CODE_FAILD,"对不起，需要：<span style='font-size:11pt;color:red'>$needname</span> 才能使用此功能。<br>你目前的等级是：<span style='font-size:11pt;color:red'>$myname</span> 。");
        }
        else if($cfg_ml->M_Money < $money)
        {
            sendResult(CODE_FAILD,"对不起，需要花费金币：<span style='font-size:11pt;color:red'>$money</span> 才能访问本页面。<br>你目前拥有的金币是：<span style='font-size:11pt;color:red'>".$cfg_ml->M_Money."</span>  。");
        }
    }
}
